...
import sqlite3
from day15_sqlite import DatabaseManager

app = FastAPI(title="SQLite Database API Example", version="1.0.0")
...
@app.get("/")
async def root():
    return {"message": "Welcome to the SQLite Database API Example!", "version": "1.0.0"}

@app.post("/users/", response_model=dict, status_code=status.HTTP_201_CREATED)
async def create_user(user: UserCreate):
    """Create a new user."""
    try:
        user_id = db.create_user(user.username, user.email, user.age)
        if user_id:
